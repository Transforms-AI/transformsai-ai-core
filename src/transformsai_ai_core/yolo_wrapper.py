...
class _YOLOExportMixin:
    """
    Shared mixin for YOLO model export and batching logic.
    Reduces code duplication between YOLOWrapper and YOLOEWrapper.
    """
    ...
    def _prepare_model(self, base_class) -> str:
        ...
        Args:
            base_class: The ultralytics base class (YOLO or YOLOE) for temporary loading
    ...
    def predict(
        ...
        **kwargs
        ...
        Args:
            ...
            base_predict: The parent class predict method to call
...
class YOLOWrapper(_YOLOExportMixin, YOLO):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        try:
            YOLO.__init__(self, model_to_load, task=self.task, **kwargs)
            ...
            if self.exported_model_path and model_to_load != str(self.original_model_path):
                ...
                YOLO.__init__(self, str(self.original_model_path), task=self.task, **kwargs)
                self.exported_model_path = None
            else:
                raise
...
class YOLOEWrapper(_YOLOExportMixin, YOLOE): 
    ...
    def __init__(
        ...
        **kwargs
        ...
        try:
            YOLOE.__init__(self, model_to_load, task=self.task, **kwargs)
            if not self.export_enabled and self.set_classes is not None:
                # If the model is not exported, set classes after loading
                YOLOE.set_classes(self, self.set_classes, YOLOE.get_text_pe(self, self.set_classes))
            self.logger.info(f"Loaded YOLOE model: {model_to_load}")
        except Exception as e:
            # Fallback to original if exported model fails
            if self.exported_model_path and model_to_load != str(self.original_model_path):
                self.logger.warning(f"Failed to load exported model, falling back to original: {e}")
                YOLOE.__init__(self, str(self.original_model_path), task=self.task, **kwargs)
                self.exported_model_path = None
            else:
                raise
